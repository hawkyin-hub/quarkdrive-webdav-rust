...
2. **AppleDouble (._) 临时文件处理红线**：macOS Finder (webdavfs_agent) 在上传具有扩展属性的文件时，必定会附带 `PUT /._filename` 请求。在 WebDAV proxy 中，**绝对不可直接返回 404 拒绝此类 PUT 请求**，否则 Finder 会判定目标存在严重错误并强行中断整个本体文件的上传（Error -36）。也不可放行其落盘到后端，否则会污染云端网盘。**必须采用 Mock 成功策略**（消费掉 Body 后返回 `201 Created` / `204 No Content`）进行无声丢弃，以此满足 Finder 的强迫症而不影响后端。
