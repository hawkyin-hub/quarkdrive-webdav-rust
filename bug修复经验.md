...
### 5. macOS Finder 拖放上传大文件报错 (-36) 且抛出 ._ AppleDouble 文件污染问题

- **现象**：
  1. 用户在 Finder 中拖拽大文件（如 `luci` 或 `clash_meta`）到挂载盘上传时，经常中途失败报错（Error -36 "can't be read or written"），且文件直接消失。
  2. 修复 Finder 报错后，用户发现虽然拖拽成功了，但云端网盘里出现了大量类似 `._luci` 的隐藏元数据垃圾文件。
- **根因**：
  1. Finder 在上传存在扩展属性（xattr）的文件时，会附带上传一个 `._` 开头的 AppleDouble 隐藏文件（如 `PUT /._luci`）。
  2. 原本 proxy 层为了防止系统隐藏文件探测拖垮后端，针对所有 `._` 文件的请求一律拦截并返回 `404 Not Found`。
  3. Finder 收到 `._` 文件上传被拒绝 (404) 后，认为目标文件系统有严重故障，进而直接中断后续真实本体文件的上传，导致用户看到传输失败、文件消失。
  4. 如果我们在后端完全放行 `._` 的上传，虽然 Finder 会上传成功，但会把这些 macOS 专属的垃圾文件实打实地落盘到了夸克云端，污染了用户的云端目录。
- **改动**：
  修改 `quarkdrive-webdav/src/proxy.rs` 中的 `._` 拦截逻辑：对探测性的 `PROPFIND` / `GET` / `HEAD` 继续返回 404；但对会触发状态变更的 `PUT` 操作，我们**主动消耗掉请求体（Drain Body）后，直接返回 `201 Created`，对 `DELETE` 同样返回 `204 No Content`**。
- **验证**：
  编译替换 proxy.rs 后，用户再次在 Finder 中拖拽上传包含扩展属性的文件，进度条顺畅跑完，Finder 不再报错 -36。登录夸克官方云端查看，不再有任何 `._` 垃圾文件残留。
- **经验**：
  面对 macOS 极其脆弱且强势的 WebDAV 客户端（webdavfs_agent），一刀切地拒绝（404）写入请求往往会导致整个事务崩溃。对于不需要落盘的垃圾数据请求，最高级的做法是**“Mock 成功（返回 2xx 状态码并丢弃数据）”**，既让强迫症 Finder 满意地继续干活，又保证了后端的纯净。
