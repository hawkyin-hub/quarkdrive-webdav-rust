...
struct UploadState {
    ...
    generation: u64,
    /// Max concurrent part uploads permitted by the OSS PDS endpoint for
    /// this upload_id, decoded from `/upload/pre` metadata.part_thread.
    /// upload_chunk caps in-flight up_part calls at this via a Semaphore
    /// so we don't get throttled to multi-minute timeouts when the server
    /// advertises part_thread=1 (which it currently does for our account).
    part_thread: u32,
}
...
struct QuarkDavFile {
    ...
    md5_ctx: Md5Context,
    sha1_ctx: Sha1,
    /// Long-lived file handle for the in-progress temp upload file. Opened
    /// lazily on the first write_buf() and dropped at upload finish, so we
    /// never fsync per write_buf chunk and never reopen the file mid-upload.
    /// The OS page cache is flushed when this handle is dropped (which
    /// happens before tokio::fs::remove_file in delete_temp_file).
    temp_file_handle: Option<tokio::fs::File>,
}
...
impl QuarkDavFile {
    ...
    async fn do_flush(&mut self) -> Result<(), FsError> {
        ...
        self.upload_state.part_thread = res.metadata.part_thread.max(1);
        let chunk_count =
    ...
    async fn consume_buf(&mut self) -> Result<(), FsError> {
        ...
        // Lazily open the temp file once and keep the handle for the rest of
        // this upload. create_dir_all / open only happen on the first
        // write_buf call, eliminating N-1 syscalls for an N-chunk write.
        if self.temp_file_handle.is_none() {
            ...
            self.temp_file_handle = Some(file);
        }
        let file = self
            .temp_file_handle
            .as_mut()
            .expect("temp_file_handle initialized above");
        file.write_all(&bytes).await.map_err(|e| {
            error!(file_name = %self.file.file_name, error = %e, "write to temp file failed");
            FsError::GeneralFailure
        })?;
        // No flush per chunk: dropping the handle at the end of the upload
        // (delete_temp_file) flushes the OS page cache, and upload_chunk
        // reads back via tokio::fs::read which sees the cached writes.
        // Incremental hashing without cloning the context state.
        let mut md5_ctx = std::mem::replace(&mut self.md5_ctx, Md5Context::new());
        let mut sha1_ctx = std::mem::replace(&mut self.sha1_ctx, Sha1::default());
    ...
    async fn upload_chunk(&mut self) -> Result<(), FsError> {
        ...
        // Pre-read all bytes once so parallel chunk futures can compute
        // offsets cheaply and avoid racing on the same file handle.
        let file_bytes = tokio::fs::read(&temp_path).await.map_err(|err| {
            error!(file_name = %self.file.file_name, error = %err, "read temp file failed");
            FsError::GeneralFailure
        })?;

        // Clone the (cheap, internally Arc-wrapped) drive so all chunk
        // futures can borrow it independently.
        let drive = self.fs.drive.clone();

        // Build per-chunk futures owned by their captured String clones.
        // `self.upload_state` is read here (immutable), not held across .await.
        let mut etags: Vec<Option<String>> = (0..chunk_count).map(|_| None).collect();
        let mut finished_early = false;
        // OSS PDS endpoint advertises max concurrent part uploads via
        // `/upload/pre` response `metadata.part_thread`. Spawning every
        // chunk as fast as we can does NOT win us anything: the server
        // only honours that many in flight at once, and any extra
        // spawns come back as `error sending request for url
        // ...partNumber=N` after long timeouts (observed: 280s for a
        // 20MB/5-chunk upload). Gate the up_part call on a permit from a
        // bounded semaphore so we never violate the advertised
        // concurrency.
        let concurrency = self.upload_state.part_thread.max(1) as usize;
        let upload_gate = std::sync::Arc::new(tokio::sync::Semaphore::new(concurrency));
        // Spawn each chunk as an independent OS task via JoinSet. This bypasses
        // futures-util `buffered`, which only polls futures cooperatively inside
        // the current task and gave no measurable speedup over serial.
        let mut joinset: tokio::task::JoinSet<Result<(u32, String), FsError>> = tokio::task::JoinSet::new();
        for chunk_idx in 1..=chunk_count {
            let start = (chunk_idx as usize - 1) * chunk_size;
            let end = std::cmp::min(start + chunk_size, file_bytes.len());
            let part_bytes = file_bytes[start..end].to_vec();
            let utc_time = chrono::Utc::now().format("%a, %d %b %Y %H:%M:%S GMT").to_string();
            let bucket_c = self.upload_state.bucket.clone();
            let obj_key_c = self.upload_state.obj_key.clone();
            let upload_id_c = self.upload_state.upload_id.clone();
            let task_id_c = self.upload_state.task_id.clone();
            let upload_url_c = self.upload_state.upload_url.clone();
            let auth_info_c = self.upload_state.auth_info.clone();
            let mime_type_c = self.upload_state.mime_type.clone();
            let drive_c = drive.clone();
            let file_name_c = file_name.clone();
            let chunk_started = std::time::Instant::now();
            let permit_fut = upload_gate.clone().acquire_owned();
            joinset.spawn(async move {
                debug!(file_name = %file_name_c, chunk_idx, "upload chunk task start");
                // Hold the permit for the whole up_part call so the next
                // chunk only starts after this one finishes.
                let _permit = permit_fut.await.expect("semaphore closed");
                let auth_meta = drive_c.up_part_auth_meta(
                    &mime_type_c, &utc_time, &bucket_c, &obj_key_c, chunk_idx as u32, &upload_id_c,
                ).await.map_err(|err| {
                    error!(file_name = %file_name_c, chunk_idx, error = %err, "get upload part auth meta failed");
                    FsError::GeneralFailure
                })?;
                let auth_res = drive_c.auth(&auth_info_c, &auth_meta, &task_id_c).await.map_err(|err| {
                    error!(file_name = %file_name_c, chunk_idx, error = %err, "auth upload part failed");
                    FsError::GeneralFailure
                })?;
                let auth_key = auth_res.data.auth_key;
                let up_req = UpPartMethodRequest {
                    auth_key: auth_key.clone(),
                    mime_type: mime_type_c,
                    utc_time: utc_time.clone(),
                    bucket: bucket_c,
                    upload_url: upload_url_c,
                    obj_key: obj_key_c,
                    part_number: chunk_idx as u32,
                    upload_id: upload_id_c,
                    part_bytes,
                };
                let res = drive_c.up_part(up_req).await.map_err(|err| {
                    error!(file_name = %file_name_c, chunk_idx, error = %err, "upload chunk failed");
                    FsError::GeneralFailure
                })?;
                let etag_from_up_part = res.ok_or_else(|| {
                    error!(file_name = %file_name_c, chunk_idx, "up_part returned None");
                    FsError::GeneralFailure
                })?;
                debug!(
                    ...
                    "upload chunk task done"
                );
                Ok::<(u32, String), FsError>((chunk_idx as u32, etag_from_up_part))
            });
        }

        while let Some(res) = joinset.join_next().await {
            let (idx, etag) = res.map_err(|err| {
                error!(file_name = %self.file.file_name, error = %err, "join upload chunk task failed");
                FsError::GeneralFailure
            })??;
            if etag == "finish" {
                finished_early = true;
            ...
            etags[(idx - 1) as usize] = Some(etag);
        }
        ...
        // After the stream is consumed, self is no longer borrowed by
        // futures; clone the fields we need to build the commit request.
        ...
        let etags_vec: Vec<String> = etags.into_iter().map(|o| o.unwrap_or_default()).collect();
        let commit_req = UpAuthAndCommitRequest{
            md5s: etags_vec,
            callback,
            bucket,
    ...
    async fn delete_temp_file(&self) -> Result<(), FsError> {
        // On macOS we can unlink an open file (POSIX semantics): the entry
        // is removed immediately, the inode persists until the last fd is
        // closed. So we don't need to take the handle here -- it's released
        // when QuarkDavFile is dropped, and upload_chunk has already read
        // the file by then. Keep the signature &self so flush()'s immutable
        // borrow of fs.upload_generation doesn't fight us.
