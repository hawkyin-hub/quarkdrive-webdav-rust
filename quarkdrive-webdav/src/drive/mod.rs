...
impl QuarkDrive {
    ...
    pub async fn rename_file(&self, file_id: &str, name: &str) -> Result<()> {
        ...
        let res: RenameFileResponse = self
            ...
            .context("expect response")?;
        if res.status != 200 {
            return Err(anyhow::anyhow!("rename file failed: {}", res.message));
        }
        Ok(())
    }
    ...
    pub async fn move_file(
        ...
        if res.status != 200 {
            return Err(anyhow::anyhow!("move file failed: {}", res.message));
        }
        Ok(())
    }
    ...
    pub async fn get_quota(&self) -> Result<(u64, u64)> {
        ...
        if res.status != 200 {
            return Err(anyhow::anyhow!("get quota failed: {}", res.message));
        }
        Ok((
    ...
    pub async fn up_pre(&self, file_name: &str, size: u64, pdir_fid: &str) -> Result<UpPreResponse> {
        ...
        if res.status != 200 {
            return Err(anyhow::anyhow!("up_pre failed: {}", res.message));
        }
        Ok(res)
    }
    ...
    pub async fn up_hash(&self, md5: &str, sha1: &str, task_id: &str) -> Result<UpHashResponse> {
        ...
        if res.status != 200 {
            return Err(anyhow::anyhow!("up_hash failed: {}", res.message));
        }
        Ok(res)
    }
    ...
    pub async fn auth(&self, auth_info: &str, auth_meta: &str, task_id: &str) -> Result<AuthResponse> {
        ...
        if res.status != 200 {
            return Err(anyhow::anyhow!("upload part auth failed: {}", res.message));
        }
        Ok(res)
    }
    ...
    pub async fn finish(&self, obj_key: &str, task_id: &str) -> Result<FinishResponse> {
        ...
        if res.status != 200 {
            return Err(anyhow::anyhow!("finish upload failed: {}", res.message));
        }
