...
impl HealthChecker {
    ...
    pub async fn check(&self) -> HealthReport {
        // Bug fix: 旧逻辑只看 sl-session 这一把 key，但用 --quark-cookie 显式传入时
        // 串里通常没有 sl-session（sl-session 只在浏览器 cookie db 里），导致 admin
        // 一律报 CookieMissing 而实际上云端读写正常。
        // 新逻辑: 数量 > 0 且至少包含一个常用关键 key (sl-session / __pus / __kp / __uid)。
        let cookies_ok = {{
            let snap = self.cookies.snapshot();
            !snap.is_empty() && snap.keys().any(|k| {{
                k == "sl-session" || k == "__pus" || k == "__kp" || k == "__uid"
                || k == "isQuark" || k == "grey-id" || k == "ctoken"
            }})
        }};
